...
# Optional section gating: LIVE_SECTIONS="L3,L4" runs only those sections
# (useful when re-validating recovery paths without re-paying L1's slow
# real-model planning calls). Default: everything.
_SECTIONS = {s.strip().upper()
             for s in os.getenv("LIVE_SECTIONS", "").split(",") if s.strip()}
...
if _sec("L1"):
    print(f"\n=== [{t()}] L1: live section ===" if _SECTIONS else "", end="")
...
if _sec("L2"):
    print(f"\n=== [{t()}] L2: live section ===" if _SECTIONS else "", end="")
...
if _sec("L34"):
    print(f"\n=== [{t()}] L34: live section ===" if _SECTIONS else "", end="")
    # ── L3: model failure -> state intact -> fallback restored -> resume ======
    print(f"\n=== [{t()}] L3: model failure / switch / resume ===")
    before_goals = len(kernel.goals)
    # Simulate a FULL reasoning outage: every configured provider gets an
    # invalid key. (Breaking only nvidia_nim no longer causes an outage --
    # the OpenRouter fallback transparently takes over, which is exactly
    # the resilience this phase added.)
    _REAL_KEYS = {}
    for _prov, _env in (("nvidia_nim", "NVIDIA_NIM_KEY"),
        ...
        _real = os.getenv(_env, "")
        if _real:
            _REAL_KEYS[_prov] = _real
            router.set_key(_prov, f"invalid-{_prov}-outage-test")
    assert _REAL_KEYS, "no provider keys in env -- cannot simulate outage"

    r3, _ = run_goal("compute 2+2 with run_code and print it", "live-3")
    check("L3 full outage handled gracefully", r3.get("success") is False,
          str(r3)[:120])
    ...
    # restore the real keys and resume (retry-tolerant like a real operator:
    # providers throw intermittent 429s/timeouts under load)
    for _prov, _key in _REAL_KEYS.items():
        router.set_key(_prov, _key)
    r4, last_err = None, ""
    _BACKOFF4 = [20, 40, 60]
...
if _sec("L5"):
    print(f"\n=== [{t()}] L5: live section ===" if _SECTIONS else "", end="")
...
if _sec("L6"):
    print(f"\n=== [{t()}] L6: live section ===" if _SECTIONS else "", end="")
    ...
    for i in range(3):
        _ep_mem.add_episode(Episode(
            # Unique per run: episodes.id is UNIQUE, so a fixed id would
            # crash every re-validation after the first (found live).
            id=f"live-ep-{int(time.time())}-{i}",
...
if _sec("L7"):
    print(f"\n=== [{t()}] L7: live section ===" if _SECTIONS else "", end="")
    ...
    with open(_srv, "w") as f:
        # dedent: this section lives under an `if _sec(...)` guard, so the
        # literal carries the guard indentation into the generated file.
        f.write(textwrap.dedent(_ECHO))
    _mcp_mgr = MCPManager()
...
if _sec("L8"):
    print(f"\n=== [{t()}] L8: live section ===" if _SECTIONS else "", end="")
