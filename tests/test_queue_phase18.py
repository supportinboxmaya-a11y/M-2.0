...
_tmp = tempfile.mkdtemp(prefix="maya_q18_")
_DB = os.path.join(_tmp, "q.db")
...
def test_inprocess_submit_still_works():
    async def go():
        q = TaskQueue(workers=2, db_path=_DB, persist=True)
        await q.start()
        out = {}
        async def job():
            out["ran"] = True
...
def test_persistent_job_runs_and_persists():
    async def go():
        ...
        q2 = TaskQueue(workers=1, db_path=_DB, persist=True)
...
def test_restart_recovery_resumes_unfinished():
    ...
    db = os.path.join(_tmp, "recover.db")

    async def phase1():
        q = TaskQueue(workers=1, db_path=db, persist=True)
...
def test_recovery_fails_orphans_without_handler():
    db = os.path.join(_tmp, "orphan.db")
...
def test_cancel_before_start():
    async def go():
        q = TaskQueue(workers=1, db_path=os.path.join(_tmp, "c.db"),
                      persist=True)
        async def slow():
...
def test_submit_job_validation():
    async def go():
        q = TaskQueue(workers=1, db_path=os.path.join(_tmp, "v.db"),
                      persist=True)
        try:
...
def test_stats_shape():
    async def go():
        q = TaskQueue(workers=3, db_path=os.path.join(_tmp, "s.db"),
                      persist=True)
